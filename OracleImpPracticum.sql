...
CONTACTID						CHAR(5 BYTE)	NOT NULL,
OWNERID							CHAR(5 BYTE) 	NOT NULL,
LAST_DATE_VERIFIED	DATE					NULL,
ISPHONE						CHAR(1 BYTE)	NOT NULL,
CONSTRAINT	P_OWNERCONTACT_PK	PRIMARY KEY(CONTACTID),
CONSTRAINT	P_OWNERCONTACT_FK	FOREIGN KEY(OWNERID)
...
CREATE TABLE P_OWNERADDRESS (
      ...
      CONSTRAINT	P_OWNERADDRESS_FK FOREIGN KEY(ADDRESS_CONTACTID)
      						REFERENCES P_OWNERCONTACT(CONTACTID),
      CONSTRAINT	P_OWNERADDRESS_STATE_CHECK CHECK
      						(STATE IN ('AL','AK','AZ','AR','CA','CO','CT','DE','FL','GA','HI','ID','IL','IN','IA','KS','KY','LA','ME','MD','MA','MI','MN','MS','MO','MT','NE','NV','NH','NJ','NM','NY','NC','ND','OH','OK','OR','PA','RI','SC','SD','TN','TX','UT','VT','VA','WA','WV','WI','WY','AS','DC','FM','GU','MH','MP','PW','PR','VI','AE','AA')),
      CONSTRAINT P_ADDRESSTYPE_CHECK CHECK
      						(ADDRESSTYPE IN ('HOME','OFFICE','RELATIVE','UNKNOWN'))
);

CREATE TABLE P_OWNERPHONE (
			PHONE_CONTACTID	CHAR(5 BYTE)			NOT NULL,
      PHONENUM					VARCHAR2(30 BYTE) NOT NULL,
      PHONETYPE					VARCHAR2(30 BYTE) DEFAULT 'UNKNOWN' NOT NULL,
      CONSTRAINT	P_OWNERPHONE_PK	PRIMARY KEY(PHONE_CONTACTID),
      CONSTRAINT	P_OWNERPHONE_FK FOREIGN KEY(PHONE_CONTACTID)
      						REFERENCES P_OWNERCONTACT(CONTACTID),
...
CREATE TABLE P_APT (
      ...
      CONSTRAINT  P_APT_FK  FOREIGN KEY(A_PROPERTYID)
...
CREATE TABLE P_HOUSE (
      ...
      CONSTRAINT  P_HOUSE_FK  FOREIGN KEY(H_PROPERTYID)
      						REFERENCES P_PROPERTY(PROPERTYID),
...
CREATE TABLE P_ISSUES (
      ...
      RESOLVED					CHAR(1 BYTE)				NULL,
      CONSTRAINT	P_ISSUES_PK		PRIMARY KEY(ISSUEID, PROPERTYID),
...
CREATE TABLE P_STATEMENTITEMS (
			STATEMENTID	CHAR(5 BYTE)	NOT NULL,
      PROPERTYID	CHAR(5 BYTE)	NOT NULL,
      CONSTRAINT P_STATEMENTITEMS_PK			PRIMARY KEY(STATEMENTID, PROPERTYID),
...
CREATE TABLE P_ACTIONS (
      ...
      OTHER												VARCHAR2(1000 BYTE)	NULL,
      CONTACTID_USED							CHAR(5 BYTE)				NULL,
      ACTION_NOTES								VARCHAR2(1000 BYTE)	NULL,
      IS_ACTION_WAITING_RESPONSE	CHAR(1 BYTE)				NULL,
      CONSTRAINT	P_ACTIONS_PK		PRIMARY KEY(ACTIONID, ISSUEID, PROPERTYID),
       CONSTRAINT   P_ACTIONS_FK1		FOREIGN KEY(ISSUEID, PROPERTYID)
      						REFERENCES P_ISSUES(ISSUEID, PROPERTYID),
      CONSTRAINT	P_ACTIONS_FK2		FOREIGN KEY(CONTACTID_USED)
      						REFERENCES P_OWNERCONTACT(CONTACTID),
