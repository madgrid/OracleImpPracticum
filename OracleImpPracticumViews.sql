...
SELECT COUNT(IS_ACTION_WAITING_RESPONSE WHEN IS_ACTION_WAITING_RESPONSE = 'Y' END) AS ACTIONS_WAITING_MORE_THAN_60,
	AVG(DISTINCT ACTIONDATE WHEN IS_ACTION_WAITING_RESPONSE = 'Y' END) AS AVERAGE_DAYS_WAITING,
  MAX(DISTINCT ACTIONDATE WHEN IS_ACTION_WAITING_RESPONSE = 'Y' END) AS MAX_DAYS_WAITING
FROM P_ACTIONS
