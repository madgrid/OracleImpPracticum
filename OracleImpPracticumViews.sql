...
SELECT P_OWNER.OWNERID, P_OWNER.LNAME, P_OWNER.FNAME, B2.PROPERTYID, B1.PURCHASEDATE
FROM P_OWNER, P_PURCHASE B1, P_PROPERTY B2
WHERE P_OWNER.OWNERID = B1.OWNERID AND NOT EXISTS
	(SELECT *
  FROM P_PURCHASE B2
...
SELECT DISTINCT TRIM(FNAME) + ' ' + TRIM(LNAME) + 'is the owner or former owner of' + COUNT(PROPERTYID) + 'properties.' AS MESSAGE
FROM P_OWNER, P_PURCHASE
WHERE P_OWNER.OWNERID = P_PURCHASE_PROPERTYID
