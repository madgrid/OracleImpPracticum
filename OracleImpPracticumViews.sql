...
CREATE VIEW P_OWNER_SUMMARY_MESSAGE_V AS
SELECT FNAME || ' ' ||  LNAME || ' is the owner or former owner of ' || COUNT(P_PURCHASE.PROPERTYID) || ' properties.' AS MESSAGE
FROM P_OWNER, P_PURCHASE
WHERE P_OWNER.OWNERID = P_PURCHASE.OWNERID
GROUP BY FNAME, LNAME;

DROP VIEW P_OWNER_SUMMARY_MESSAGE_V;


CREATE VIEW P_OWNERS_WHO_HAVE_OWNED_ALL_V AS
SELECT DISTINCT P_OWNER.OWNERID, LNAME, FNAME, SSN, EMAIL
FROM P_OWNER
WHERE NOT EXISTS
	(SELECT P_PURCHASE.OWNERID, P_PURCHASE.PROPERTYID
  FROM P_PURCHASE
  WHERE NOT EXISTS
  	(SELECT P_PROPERTY.PROPERTYID
    FROM P_PROPERTY
    WHERE P_PURCHASE.PROPERTYID = P_PROPERTY.PROPERTYID AND P_OWNER.OWNERID = P_PURCHASE.OWNERID));

DROP VIEW P_OWNERS_WHO_HAVE_OWNED_ALL_V;
